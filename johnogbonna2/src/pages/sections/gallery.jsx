...
function Gallery(props) {
    ...
    let [category, setCategory] = useState(searchParams.get('category') ? searchParams.get('category') : 'All');
            ...
            <div className='Gallery__photos'>
                {category === 'All' ?
                    photos.map((image, index) => {
                        return (
                            <div className='photoBox' key={`${category}-${index}`}>
                                <img className='photogrid__photo' loading='lazy' src={image.picture} onClick={() => setupLargeView(photos, index)} alt='all display' />
                            </div>
                        )
                    })
                    : null}
                {category === 'Grad' ?
                    gradPhotos.map((image, index) => {
                        return (
                            <div className='photoBox' key={`${category}-${index}`}>
                                <img className='photogrid__photo' loading='lazy' src={image.picture} onClick={() => setupLargeView(gradPhotos, index)} alt='grad display' />
                            </div>
                        )
                    })
                    : null}
                {category === 'Family' ?
                    familyPhotos.map((image, index) => {
                        return (
                            <div className='photoBox' key={`${category}-${index}`}>
                                <img className='photogrid__photo' loading='lazy' src={image.picture} onClick={() => setupLargeView(familyPhotos, index)} alt='family display' />
                            </div>
                        )
                    })
                    : null}
                {category === 'Self' ?
                    selfPhotos.map((image, index) => {
                        return (
                            <div className='photoBox' key={`${category}-${index}`}>
                                <img className='photogrid__photo' loading='lazy' src={image.picture} alt='self display' onClick={() => setupLargeView(selfPhotos, index)} />
                            </div>
                        )
                    })
                    : null}
                {category === 'Other' ?
                    otherPhotos.map((image, index) => {
                        return (
                            <div className='photoBox' key={`${category}-${index}`}>
                                <img className='photogrid__photo' loading='lazy' src={image.picture} onClick={() => setupLargeView(otherPhotos, index)} alt='other display' />
                            </div>
                        )
                    })
                    : null}
